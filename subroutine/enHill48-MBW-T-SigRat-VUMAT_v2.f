...
c SDV31: Equivalent plastic strain rate                                            c
c SDV32: Increment counter (for eq. plas. str. rate computation)                   c
c SDV33: Sum of equivalent plastic strain increments                               c
c        before the computation of eq. plas. str. rate                             c
c SDV34: Sum of time increments before computation of eq. plas. str. rate          c
c SDV35: Additive strain rate hardening correction function                        c
c==================================================================================c
...
C Defining numerical constants
     ...
     1  rEB,F,G,H,L,M,N,F1,G1,H1,L1,M1,N1,T0,cT1,cT2,cT3,Dens,Cp,TQCbeta,
...
C Update temperature    
       T= stateOld(i,3)
       if(T.le.Tol .and. T.ge.(zero-Tol)) then
         T = T0 + (TQCbeta/(Dens*Cp))*radius*deqplas/facDctDamDev
       else 
         T = T + (TQCbeta/(Dens*Cp))*radius*deqplas/facDctDamDev
       endif 
       facT = cT1*exp(zero-cT2*T)+cT3	   
...
c Ductile damage initiation is suppressed when Eta<Cut-off value 
       DDII = stateOld(i,16)
!       if(DDII.le.Tol .and. DDII.ge.(zero-Tol)) then
!       DDII = zero
!       endif
       if(Eta.lt.Cut) then
       DDII = DDII
       endif
       dDDII = deqplas/Epsddi
       if(Eta.ge.Cut) then
       DDII = DDII+dDDII
       endif
...
C Calculate the ductile fracture indicator 
       DFI=stateOld(i,17)
       Damage= stateOld(i,18)	   
       if(DFI.le.Tol .and. DFI.ge.(zero-Tol)) then
       DFI = zero
       endif 	
C Calculate the yield stress at the onset of ductile damage
       if(DDII.ge.one .and. eqplas.gt.Tol) then
         Sigdi=stateOld(i,27)
         Epsdi=stateOld(i,28)
...
c Elements under hydrostatic compression don't experience spherical damage
        if(Eta.lt.Cut .and. DFI.lt.one) then
           DFI=DFI
           dDFI=zero
           Damage= Damage
           dDamage = zero		   
        else
           dDFI=Sigdi*deqplas/(Gf*Dcrit)
           DFI=DFI+dDFI
...
C Cleavage fracture with both stress and strain criteria
       CFI = stateOld(i,13)
       if(CFIE.ge.one .and. eqplas.gt.zero) then
         if (CFIS.ge.one .and. eqplas.gt.zero) then
           STRESST(1) = zero
           STRESST(2) = zero
           STRESST(3) = zero
           STRESST(4) = zero
